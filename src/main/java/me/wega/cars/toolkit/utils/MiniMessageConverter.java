...
public class MiniMessageConverter {
    ...
    public final class MiniTranslator {
        private static final Set<Option> DEF_OPTIONS = Collections.unmodifiableSet(EnumSet.of(
                Option.COLOR, Option.HEX, Option.FORMAT, Option.GRADIENT, Option.FAST_RESET
        ));
        ...
        private static @Nullable String tagByChar(char ch, Collection<Option> options) {
            if (('0' <= ch && ch <= '9') || ('a' <= ch && ch <= 'f') || ch == '#' || ch == 'x') {
                if (options.contains(Option.HEX) && (ch == 'x' || ch == '#')) return "color";
                if (options.contains(Option.COLOR)) return colorByChar(ch);
        ...
        public enum Option {
            /**
             * Translate color (e.g. &a &1)
             */
            COLOR,
            /**
             * Translate hex color (e.g. &#rrggbb)
             */
            HEX,
            /**
