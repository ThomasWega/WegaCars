...
import me.wega.cars.toolkit.utils.ComponentUtils;
...
/**
 * A simple builder to create ItemStack with a fluent API
 */
@ToString
@EqualsAndHashCode
public class ItemBuilder {
    ...
    public ItemBuilder displayName(Component displayName) {
        ...
        this.displayName = ComponentUtils.decorationIfAbsent(displayName, TextDecoration.ITALIC, TextDecoration.State.FALSE);
    ...
    public ItemBuilder lore(@NotNull List<? extends Component> lore) {
        ...
        // in vanilla minecraft italic may be set on some items
        newLore.replaceAll(component -> ComponentUtils.decorationIfAbsent(component, TextDecoration.ITALIC, TextDecoration.State.FALSE));
        this.lore = newLore;
        meta.lore(this.lore);
        itemStack.setItemMeta(meta);
        return this;
     ...
     *
     * @param lines The text to display on lore
     * @return The ItemBuilder
     */
    public ItemBuilder appendLoreLine(Component... lines) {
        lore.addAll(List.of(Arrays.stream(lines)
                .map(line -> ComponentUtils.decorationIfAbsent(line, TextDecoration.ITALIC, TextDecoration.State.FALSE))
    ...
    public ItemBuilder appendLore(Collection<Component> lines) {
    ...
    public ItemBuilder setLoreLine(int index, Component line, boolean override) {
        line = ComponentUtils.decorationIfAbsent(line, TextDecoration.ITALIC, TextDecoration.State.FALSE);
        if (override) {
            lore.set(index, line);
        } else {
            lore.add(index, line);
    ...
    public ItemBuilder setLoreLine(int index, Component line) {
        return setLoreLine(
                index,
                ComponentUtils.decorationIfAbsent(line, TextDecoration.ITALIC, TextDecoration.State.FALSE),
                true
        );
    }
    ...
    /**
     * Build the item to a skull with the given UUID
     * @param uuid The UUID of the skull
     * @return The ItemStack
     */
    public ItemStack buildToSkull(@NotNull UUID uuid) {
        this.itemStack.setType(Material.PLAYER_HEAD);
        return SkullCreator.itemWithUuid(this.itemStack, uuid);
    }
